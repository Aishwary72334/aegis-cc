...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('[AEGIS Register Error]', err);
      let msg = 'Failed to request authorization.';
      if (err) {
    ...
    <div className="min-h-screen bg-hud-bg flex items-center justify-center p-4 font-sans hud-scanline-effect">
      ...
      <div className="w-full max-w-md hud-glass p-8 rounded-2xl relative border border-hud-border shadow-2xl">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-hud-muted mt-1 font-mono tracking-widest">Access Registration</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1">
            <label className="block text-xs uppercase tracking-widest font-mono text-hud-muted">Full Name</label>
            <input
              type="text"
              value={fullName}
              ...
              className="w-full bg-slate-950/50 border border-hud-border focus:border-hud-accent rounded-lg px-4 py-2.5 text-hud-text font-mono text-sm placeholder-hud-muted/50 outline-none transition-all duration-300 shadow-inner"
            />
          </div>

          {/* Email field */}
          <div className="space-y-1">
            <label className="block text-xs uppercase tracking-widest font-mono text-hud-muted">System Email</label>
            <input
              type="email"
              value={email}
          ...
          <div className="space-y-1 relative">
            ...
            <div className="relative">
              <input
                ...
                required
                placeholder="••••••••••••"
                className="w-full bg-slate-950/50 border border-hud-border focus:border-hud-accent rounded-lg pl-4 pr-12 py-2.5 text-hud-text font-mono text-sm placeholder-hud-muted/50 outline-none transition-all duration-300 shadow-inner"
              />
              <button
                type="button"
          ...
          {/* Role selection */}
          <div className="space-y-1">
            <label className="block text-xs uppercase tracking-widest font-mono text-hud-muted">Security clearance / Role</label>
            <select
              value={role}
              onChange={(e) => setRole(e.target.value)}
              className="w-full bg-slate-950/90 border border-hud-border focus:border-hud-accent rounded-lg px-4 py-2.5 text-hud-text font-mono text-sm outline-none transition-all duration-300 shadow-inner cursor-pointer"
            >
              <option value="Owner">Owner (Full access)</option>
              <option value="Manager">Manager (Edit schedules/tasks)</option>
        ...
        <div className="mt-8 text-center border-t border-hud-border/40 pt-6">
          <p className="text-xs text-hud-muted font-mono">
            Already authorized?{' '}
            <Link to="/login" className="text-hud-accent hover:text-hud-accent/80 transition-colors ml-1 font-semibold underline decoration-hud-accent/40 hover:decoration-hud-accent">
              Initiate Session
            </Link>
          </p>
        </div>
